...
SELECT employee.f_name,
    employee.l_name,
    SUM(product.unit_price * orderItem.quantity) AS "total_sales(BATH)" -- sum of quantity mutiply by unit price
FROM employee
...
HAVING "total_sales(bath)" > 1000 -- get only employee who makes the most profit more than 1000
ORDER BY "total_sales(bath)" DESC -- order by descending
